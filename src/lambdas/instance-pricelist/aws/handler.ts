...
const truncateAwsInstancePricingData = (pricingResponseArray: Product[]): Response[] => {
    let pricingResponses: Response[] = [];

    //Filter Data
    var filteredPricingData = _.filter(pricingResponseArray, function (prod) {
        return (prod.product?.attributes?.tenancy == "Shared" &&
    ...
    _.each(filteredPricingData, function (prod) {
        ...
        for (const [reservedKey, reservedValue] of _.entries(
            ...
            for (const [priceDimensionKey, priceDimensionValue] of _.entries(
                ...
                if (priceDimensionValue.unit == "Quantity" || priceDimensionValue.description == "Upfront Fee") {
        ...
        pricingResponses.push({
            ...
            costs: costs,
            normalizationSizeFactor: prod.product?.attributes?.normalizationSizeFactor
        })
    })
